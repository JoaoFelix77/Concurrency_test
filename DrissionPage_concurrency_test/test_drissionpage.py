...
def fetch(url, mode):
    try:
        if mode == 'miniblink':
            co = ChromiumOptions()
            co.set_browser_path('miniblink')  # 指定使用miniblink
            ...
            # 其他 Chromium 模式（chrome / chrome-no-js / chrome-no-media）
            co = ChromiumOptions()
            co.remote_port = 9222
            page = ChromiumPage(co)
            ...
            if mode == 'chrome-no-js':
                co = ChromiumOptions()
                co.set_argument('--disable-javascript')               #禁js
                page = ChromiumPage(co)
            elif mode == 'chrome-no-media':
                co = ChromiumOptions()
                co.set_browser_path('chrome')
                co.set_argument('--blink-settings=imagesEnabled=false')  # 禁图
                co.set_argument('--disable-plugins')  # 禁插件
                page = ChromiumPage(co)
